...
class CameraApp(App):
    ...
    def build(self):
        ...
        self.capture = cv2.VideoCapture(1)  #criamos um objeto de capture de vídeo. Associamos à primeira camera
        ret, frame = self.capture.read() #criamos um frame com esta imagem
        #cv2.namedWindow("CV2 Image")     #nome da janela
        #cv2.imshow("CV2 Image", frame)
        Clock.schedule_interval(self.update, 1.0/30.0)   #criamos um clock para atualizar a imagem a cada 1/320 de segundo
        return layout
    ...
    def update(self, dt):
        ...
        self.texture = texture1 #apresenta a imagem
