...
class KivyCamera(Image):
    ...
    #         self.to_parent(self.circle_x, self.circle_y)))
    #     return vals
    ...
    def update(self):
        ...
        for (i, face) in enumerate(faces):
            ...
            (x, y, w, h) = face_utils.rect_to_bb(face)  # x = col, y = row
            # for (x, y) in shape:
            ...
            # self.circle_pos = self.to_parent(circle_x, circle_y)
            # self.circle_pos = self.to_parent(x, y)
            self.circle_pos = self.to_parent(x, cy)
